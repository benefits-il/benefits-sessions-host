...
- [ ] טאבים פתוחים מראש: קונסולת מפגש 4 · שלושת הדקים · עמוד המפגש באתר התלמידים · אתר הדוגמאות (benefits-il.dev/pages-gallery) — נבדק שהוא חי וטעון, לא נפתח לוקאלית בשיעור · tuesday (עמוד הבית, מערכת העיצוב, עמוד המוצר, הדמו לפני/אחרי) · תיקיית TUESDAY פתוחה ב-VS Code.
...
| 00:40–01:25 | בניית עמוד המוצר + הפסקה חופפת ל-build | VS Code + תיקיית TUESDAY, דפדפן |
...
- ‏[חובה] מצב Bypass permissions: כברירת מחדל קלוד עוצר ומבקש אישור כמעט על כל פעולה. במצב הזה הוא עובד ברצף עד הסוף בלי לעצור. מדליקים אותו כשעובדים בתוך תיקיית פרויקט מבודדת — במקרה הגרוע מוחקים את התיקייה ומתחילים מחדש. בתיקייה עם קבצים חשובים נשארים במצב הרגיל. את המסלול המדויק להפעלה מראים כאן, כי כולם יעברו אותו: Ctrl יחד עם פסיק פותח את ההגדרות, מחפשים "bypass", נכנסים ל-Extensions ואז ל-Claude Code, ומסמנים את "Allow bypass permissions".
- ‏[חובה] החלפת מודלים: יש כמה מודלים — מהיר לעבודה השוטפת, חזק למשימות מסובכות באמת. מחליפים בבורר המודל שליד מחוון-המצב, או בפקודה ‎/model. המודל החזק גם צורך יותר מהמכסה — לכן ברירת המחדל לרוב, והחזק כשנתקעים או כשהמשימה באמת כבדה. חריג של היום: שני ה-build-ים הכבדים — עמוד המוצר ומערכת העיצוב — רצים על Opus בכוונה, כי אלה המשימות הכבדות באמת. הכלל «שומרים את החזק לרגעים שצריך» חל על שאר השיחות.
...
## 00:40–01:25 · בניית עמוד המוצר + הפסקה חופפת ל-build
...
- **מה עושים:**
  ...
  2. מדביקים את הפרומפט בקלוד קוד, במצב Plan Mode, בתוך תיקיית המוצר, ומוודאים שהמודל הוא Opus. קלוד ישאל שפה, יציג תוכנית, ויחכה לאישור.
  3. מאשרים את התוכנית ומחליפים חזרה ל-Bypass permissions (Shift יחד עם Tab) כדי שהבנייה תרוץ ברצף עד הסוף בלי לעצור על כל אישור.
  4. **הבנייה ארוכה — לא מחכים מולה.** משגרים את ה-build אצל כולם, ויוצאים להפסקה מובנית של 10 עד 15 דקות בזמן שהוא רץ. מי שנתקע באחת ההכנות משלים עכשיו, עם שכן או איתי.
  5. חוזרים מההפסקה: מסיירים קצר בעמוד TUESDAY הגמור בדפדפן ומראים בדיוק מה נוצר אצלם, ואז פותחים תוצר של אחד הלומדים ופותחים בדפדפן — גוללים קדימה ואחורה.
...
- **לשים לב:** סרטון לא נמצא — בודקים שם ותיקייה. עמוד לבן — פותחים את קונסולת הדפדפן ומדביקים לקלוד את השורה האדומה הראשונה. חילוץ פריימים נכשל — מריצים שוב את בדיקת ffmpeg מהפרומפט. תוצאה גנרית — מפנים את קלוד חזרה לקובץ המותג. תקרת זמן: ב-01:25 עוברים למערכת עיצוב גם אם לא כולם סיימו — נקודת ההמשך כתובה בעמוד הכיתה.
...
- **מה אומרים:** בשיעור שעבר כל אות הייתה כלי בפני עצמו; היום מפעילים לראשונה שרשרת שלמה. «אות בודדת היא מילה; compound הוא משפט.» «והקסם: אתם לא כותבים אף פרומפט. הסקיל כותב לכם את כולם, שלב אחרי שלב.» «שני הסקילים האלה הם לא קסם. הם בנויים בדיוק מהאותיות שאתם כבר מכירים.» מבהירים איפה שני הסקילים המורכבים: kb-builder, שאותו מפעילים עכשיו ובונה מאגר ידע, ו-design-pipeline, שבונה מערכת עיצוב שלמה — אותו תריצו בשיעורי-הבית. ומראש: המאגר שמתחילים היום, יחד עם העיצוב, ישמשו אתכם בפרויקט הסיום.
- **מה עושים:**
  1. פתיחה קצרה (2 דק').
...
לקראת השיעור ביום שלישי, שתי הכנות קטנות:
1. ודאו שסיימתם את שלוש יחידות ההכנה — העמוד שלכם באוויר, הסרטון שמור אצלכם, ו-ORBIT מותקן בקלוד קוד.
